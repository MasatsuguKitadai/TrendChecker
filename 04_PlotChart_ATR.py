...
# ★変更点：読込先と出力先を「ATR用」に変更
RESULT_DIR = "simulation_results_atr"
OUTPUT_CHARTS_DIR = "visualized_charts_atr"
SIM_MONTHS = 6
...
def visualize_all_analyzed_data():
    ...
    for file_name in all_files:
        ...
        # ★変更点：タイトルに「ATR」と明記
        fig.update_layout(title=f"{ticker} 複利運用レポート (ATR可変・累積率ベース)", template="plotly_white", height=1100, xaxis_rangeslider_visible=False)
        fig.write_html(os.path.join(OUTPUT_CHARTS_DIR, f"chart_{ticker}.html"))
        print(f"◎ {ticker}: ATRチャート生成完了")
