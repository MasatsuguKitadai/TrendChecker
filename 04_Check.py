...
def visualize_all_analyzed_data():
    ...
    for file_name in all_files:
        ...
        fig.add_trace(go.Scatter(x=df.index, y=df['MA_Long'], name="MA75", line=dict(color='green', width=1)), row=1, col=1)
        ...
        if not trades.empty:
            for _, row in trades.iterrows():
                # 買い(BUY)は赤、売り(SELL)は緑、保有中(HOLDING)はピンクで垂直線を引く
                if row['Action'] in ["BUY_SIGNAL","HOLDING"]:
                    line_color = "#dc3545"  # 買いの色
                    line_dash = "dash"
                elif row['Action'] in ["BUY", "BUYBACK"]:
                    line_color = "#dc3545"  # 買いの色
                    line_dash = "dash"
                else:
                    line_color = "#28a745"  # 売りの色
                    line_dash = "dash"

                # V線を引く
                fig.add_vline(x=row['Date'], line_width=1.2, line_dash=line_dash, line_color=line_color, row=1, col=1)

        fig.update_xaxes(rangebreaks=[dict(bounds=["sat", "mon"])])
        fig.update_layout(title=f"{ticker} ロング解析 (直近6ヶ月)", template="plotly_white", height=800, xaxis_rangeslider_visible=False)
